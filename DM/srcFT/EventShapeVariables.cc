...
EventShapeVariables::EventShapeVariables(const std::vector<TLorentzVector>& inputVectors)
{
  inputVectors_.reserve( inputVectors.size() );
  for(std::vector<TLorentzVector>::const_iterator vec = inputVectors.begin(); vec != inputVectors.end(); ++vec ){
